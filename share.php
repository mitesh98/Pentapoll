...
//$poll_id="843362";
$login = isset($_SESSION['emobile']);
$q="SELECT * FROM `polldetail` WHERE poll_id='$poll_id'";
...
if(!$login)
{
 ...
 <div class="custom-padding-1" id="main-content">
<div class="col-md-3"></div>
 <div class="panel panel-default col-md-6">
 <div class="panel-body custom-panel-body-last">
<div id="write"></div>
   <p class="question"><?=$row['question'] ?></p>
	...
	{
	?>
     <div class="question-image" id="poll_pic">
    <img class="img-responsive"  src="<?= $row['poll_pic']?>"/>
       </div>
	<?php
	}
	?>
    ...
    <input type="radio" id="q1op1" name="option" onclick=" opt=<?=$poll_id?>;opt=(opt*10)+1;"><label for="q1op1">&nbsp;<?=$row['option1']?></label>
      </div>
                    <div class="col-lg-4 col-md-4 col-xs-4">
                        <input type="radio" id="q1op2" name="option" onclick=" opt=<?=$poll_id?>;opt=(opt*10)+2;"><label for="q1op2">&nbsp;<?=$row['option2'] ?></label>
                    </div>
					<?php
					if($row['option3'] !='')
					{
						?>
                    <div class="col-lg-4 col-md-4 col-xs-4">
                        <input type="radio" id="q1op3" name="option" onclick=" opt=<?=$poll_id?>;opt=(opt*10)+3;"><label for="q1op3">&nbsp;<?=$row['option3'] ?> Plus</label>
                    </div>
					<?php
					}
							?>
                ...
                <div class="row text-center">
                    ...
                    <div class="col-lg-4 col-md-4 col-xs-4">
                        <input type="radio" id="q1op4" name="option" onclick="var opt=<?=$poll_id?>;opt=(opt*10)+4;"><label for="q1op4">&nbsp;<?=$row['option4'] ?></label>
                    </div>
					<?php
					}
							?>
					...
					if($row['option5'] !='')
					{
						?>
                    <div class="col-lg-4 col-md-4 col-xs-4">
                        <input type="radio" id="q1op5" name="option" onclick="opt=<?=$poll_id?>;opt=(opt*10)+5;"><label for="q1op5">&nbsp;<?=$row['option5']?></label>
                    </div>
					<?php
					}
							?>
            ...
            <div class="panel-footer text-center" style="border:none;"><button class="custom-button-1" id="poll_button<?=$poll_id?>" onclick="option=opt;">Poll it</button></div>
			 </div>
			 <div class="col-md-3"></div>
	</div>
...
<!------------------------------------------------->
	...
	<script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
        <div class="panel panel-default">
				<div class="col-md-3"></div>
				<div class="col-md-6">
            <div class="panel-heading custom-panel-heading-last">
                <div class="media-left">
                    <img src="<?= $rowc['pic'];?>" onerror="this.src='images/head.png'" class="media-object img-circle custom-border-1" style="width: 4rem;height:4rem;">
                </div>
                <div class="media-body">
...
var login='<?=$login?>';
	//alert(login);
	$("<?='#poll_button'.$poll_id?>").click(function(){
	if(!login)
	{
		//alert(opt);
                 var poll_id=option;
		$.post("share_signup_login.php",{poll_id:poll_id},function(data){
		 $("#write").html(data);
		 });
	}
	else if(login)
	{
		//alert(opt);
		$.get("count3.php",{opt:opt},function(data){
		 $("#write").html(data);
		 });
	}
